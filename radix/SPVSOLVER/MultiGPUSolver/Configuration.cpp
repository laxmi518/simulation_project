...
CCTStatusType CConfiguration::ReadConfigurationFileWithDrag(const std::string& FileName)
{
	...
	while(!Line.empty())
	{
		...
		if(IfDrag=="D" || IfDrag=="DE" || IfDrag == "DC" || IfDrag == "DF")
		{
			...
			bool NegativePressureCheck = false;
			m_aNegativePressureCheck.push_back(NegativePressureCheck);

			Scalar InflueneRegion=0;
			m_aCoefficientofInflueneRegion.push_back(InflueneRegion);

			Scalar3 tempDirectionVector = make_Scalar3(0,0,0);
			m_aDirectionVector.push_back(tempDirectionVector);

			Scalar PressureGas=0;
			m_aPressureGas.push_back(PressureGas);
		...
		else
		{
			...
			bool NegativePressureCheck = false;
			LineStream >> NegativePressureCheck ; //Read Either Check for Negative Pressure or not
			m_aNegativePressureCheck.push_back(NegativePressureCheck);

			Scalar InflueneRegion=0;
			LineStream >> InflueneRegion;  //Read Influene Region;
			m_aCoefficientofInflueneRegion.push_back(InflueneRegion);

			Scalar3 tempDirectionVector = make_Scalar3(0,0,0);
			LineStream >> tempDirectionVector.x >> tempDirectionVector.y >> tempDirectionVector.z;  //Read direction vector
			m_aDirectionVector.push_back(tempDirectionVector);

			Scalar PressureGas=0;
			LineStream >> PressureGas; //Read Individual Pressure gas value for STLs
			m_aPressureGas.push_back(PressureGas);
...
bool CConfiguration::GetNegativePressureCheck(unsigned int Model) const
{	
	if(Model < m_aNegativePressureCheck.size() && Model >= 0)
	{
		return m_aNegativePressureCheck[Model];
	}
	else
	{
		bool temp = false;
...
Scalar CConfiguration::GetInflueneRegion(unsigned int Model) const
{	
	if(Model < m_aCoefficientofInflueneRegion.size() && Model >= 0)
	{
		return m_aCoefficientofInflueneRegion[Model];
	}
	else
	{
		Scalar temp = 0;
...
Scalar3 CConfiguration::GetDirectionVector(unsigned int Model) const
{	
	if(Model < m_aDirectionVector.size() && Model >= 0)
	{
		return m_aDirectionVector[Model];
	}
	else
	{
		Scalar3 temp = {0,0,0};
...
Scalar CConfiguration::GetPressureGas(unsigned int Model) const
{	
	if(Model < m_aPressureGas.size() && Model >= 0)
	{
		return m_aPressureGas[Model];
	}
	else
	{
		Scalar temp = 0;
...
const vector<Scalar>& CConfiguration::GetInflueneRegion() const
{
	return m_aCoefficientofInflueneRegion;
}
const vector<Scalar3>& CConfiguration::GetDirectionVector() const
{
	return m_aDirectionVector;
}
