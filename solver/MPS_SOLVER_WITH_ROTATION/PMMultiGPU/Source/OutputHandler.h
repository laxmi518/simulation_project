...
public:
	...
	~COutputFrame()
	{
		...
		{
			cudaFreeHost(m_aOutputParticleID);
		}
		if(m_aOutputParticlePosition)
		{
			cudaFreeHost(m_aOutputParticlePosition);
		}
		if(m_aOutputParticleVelocity)
		{
			cudaFreeHost(m_aOutputParticleVelocity);
		}
		if(m_aOutputParticlePressure)
		{
			cudaFreeHost(m_aOutputParticlePressure);
		}
		if(m_aOutputParticleDensity)
		{
			cudaFreeHost(m_aOutputParticleDensity);
		}
		if(m_aOutputParticleTemperature)
		{
			cudaFreeHost(m_aOutputParticleTemperature);
		}
		if(m_aOutputParticleKineticViscosity)
		{
			cudaFreeHost(m_aOutputParticleKineticViscosity);
		}
		if(m_aOutputParticleSolidPhaseRate)
		{
			cudaFreeHost(m_aOutputParticleSolidPhaseRate);
		}
		if(m_aOutputParticleType)
		{
			cudaFreeHost(m_aOutputParticleType);
		}
	}
	...
	void SetOutputParams(Integer ParticleNum,Scalar AnalysisTime,std::string& Path)
	{
		...
		m_FileName = Path;
		//m_RotateAngleTimes = rotationTimes;
	}
	/*
	void SetModelPosition(Integer Step, Scalar dt, const std::vector<Scalar3>& ModelVelocity, const std::vector<Scalar3>& InitialModelPosition)
	{
		...
		for(Integer i = 0;i < m_ModelPosition.size();++i)
		{
