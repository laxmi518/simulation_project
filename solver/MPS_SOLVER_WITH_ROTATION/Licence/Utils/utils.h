...
*/
    ...
    bool RsaVerifyVector(const std::string & publicKeyStr, const std::string& vec, const std::vector<char>& sign); //Used for Client
    void SaveResToVector(const std::wstring& resName, int resId, std::vector<char>* target);//Used for Client
	void LoadFileToVector(HANDLE hFile, std::vector<char>& data);
	void LoadFileToVector(const std::wstring& fileName, std::vector<char>& data);//Used for Client
