...
static __inline__ __device__ __host__  float  sqrt14(const float x)
{
...
{
	return sqrt14((lhs->x - rhs->x) * (lhs->x - rhs->x) + (lhs->y - rhs->y) * (lhs->y - rhs->y) + (lhs->z - rhs->z) * (lhs->z - rhs->z));
}
...
static __inline__ __device__ __host__ Scalar3 CalcNormalDistance(const CTriangle* const pTriangle, const Scalar3* const Position)
{
	...
	const Scalar sDenom = sqrt14(denominator);
...
static __inline__ __device__ __host__ Scalar GetZValueFunctionLaplacian(const Scalar InterpolatedDistance, const Scalar InitialParticleDistance)
{
	...
	Scalar Value = a * (InterpolatedPerInitial - b) + c;
	return Value;
}
...
{	
	...
	Scalar NumericConst = sqrt14(1.0/3.0);
